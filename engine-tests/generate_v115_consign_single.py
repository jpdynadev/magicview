...
DST = DECKS / 'Kinnan_ARCH_F10_CONSIGN_SINGLE.dck'
...
assert 'Consign to Memory' not in cards
cards.remove('Energy Refractor')
cards.append('Consign to Memory')
assert len(cards) == 99 and len(set(cards)) == 99
header[1] = 'Name=Kinnan ARCH F10 Consign Single'
DST.write_text('\n'.join(header + [f'1 {c}' for c in cards]) + '\n')
print('F10_CONSIGN_SINGLE CHANGES 1 CUT Energy Refractor ADD Consign to Memory')
