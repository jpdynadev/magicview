...
replacement = "    'Mental Misstep','Mindbreak Trap','Daze','Disrupting Shoal','Pyroblast','Red Elemental Blast','Deflecting Swat','Deadly Rollick',\n"
if 'Disrupting Shoal' not in s:
    if needle not in s:
        raise SystemExit('interaction insertion point not found')
    s = s.replace(needle, replacement, 1)
p.write_text(s)
print('registered Disrupting Shoal as interaction')
