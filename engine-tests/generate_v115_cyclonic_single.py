...
DST = DECKS / 'Kinnan_ARCH_F10_CYCLONIC_SINGLE.dck'
...
assert 'Cyclonic Rift' not in cards
cards.remove('Energy Refractor')
cards.append('Cyclonic Rift')
assert len(cards) == 99 and len(set(cards)) == 99
header[1] = 'Name=Kinnan ARCH F10 Cyclonic Single'
DST.write_text('\n'.join(header + [f'1 {c}' for c in cards]) + '\n')
print('F10_CYCLONIC_SINGLE CHANGES 1 CUT Energy Refractor ADD Cyclonic Rift')
