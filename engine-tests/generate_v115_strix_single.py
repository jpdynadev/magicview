...
DST = DECKS / 'Kinnan_ARCH_F10_STRIX_SINGLE.dck'
...
assert 'Strix Serenade' not in cards
cards.remove('Energy Refractor')
cards.append('Strix Serenade')
assert len(cards) == 99 and len(set(cards)) == 99
header[1] = 'Name=Kinnan ARCH F10 Strix Single'
DST.write_text('\n'.join(header + [f'1 {c}' for c in cards]) + '\n')
print('F10_STRIX_SINGLE CHANGES 1 CUT Energy Refractor ADD Strix Serenade')
